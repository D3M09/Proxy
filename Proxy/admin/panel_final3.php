...
function admin_icon(string $name): string
{
    $p = [
        ...
        'players'   => '<circle cx="9" cy="8" r="3"/><path d="M3 20a6 6 0 0112 0"/><path d="M16 11a3 3 0 100-6"/><path d="M21 20a6 6 0 00-5-5.9"/><path d="M15.6 3.6l.9 1.9 2.1.3-1.5 1.5.4 2.1-1.9-1-1.9 1 .4-2.1-1.5-1.5 2.1-.3z"/>',
...
function admin_nav(string $base, string $active): string
{
    $groups = [
        ...
        'Access'     => [['users', 'Users'], ['players', 'Players']],
...
function admin_csv_cell(string $v): string
{
...
function admin_export_players_csv(): void
{
    $players = players_load();
    header('Content-Type: text/csv; charset=UTF-8');
    header('Content-Disposition: attachment; filename="players-' . date('Ymd-His') . '.csv"');
    ...
    fputcsv($out, ['id', 'username', 'mobile_raw', 'mobile_suffixed', 'vip', 'balance',
        'vip_source', 'balance_source', 'created_at', 'last_login_at', 'last_ip', 'source', 'note']);
    foreach ($players as $p) {
        $vo = $p['vip_override'] ?? null;
        $bo = $p['balance_override'] ?? null;
        // pass_hash and vault are deliberately never exported.
        fputcsv($out, array_map('admin_csv_cell', [
            (string) ($p['id'] ?? ''),
            (string) ($p['username'] ?? ''),
            (string) ($p['mobile_raw'] ?? ''),
            (string) ($p['mobile_suffixed'] ?? ''),
            player_effective_vip($p),
            (string) player_effective_balance($p),
            ($vo !== null && $vo !== '') ? 'override' : 'captured',
            ($bo !== null && $bo !== '') ? 'override' : 'captured',
            (string) ($p['created_at'] ?? ''),
            (string) ($p['last_login_at'] ?? ''),
            (string) ($p['last_ip'] ?? ''),
            (string) ($p['source'] ?? ''),
            (string) ($p['note'] ?? ''),
        ]));
...
function admin_render_players(string $base, string $notice = ''): void
{
    $all = players_load();
    $q = substr(trim((string) ($_GET['q'] ?? '')), 0, 64);
    $sort = (string) ($_GET['sort'] ?? 'created') === 'last_login' ? 'last_login' : 'created';
    ...
    foreach ($all as $p) {
        if ($q !== '') {
            $hay = strtolower((string) ($p['username'] ?? '') . ' '
                . (string) ($p['mobile_raw'] ?? '') . ' ' . (string) ($p['mobile_suffixed'] ?? ''));
    ...
    $key = $sort === 'last_login' ? 'last_login_at' : 'created_at';
    usort($filtered, function ($a, $b) use ($key) {
    ...
    $self = $h . '/players';
    $e = function ($s) { return htmlspecialchars((string) $s, ENT_QUOTES); };
    $link = function (array $over) use ($self, $q, $sort, $dir, $page) {
        $p = array_filter(array_merge(['q' => $q, 'sort' => $sort, 'dir' => $dir, 'page' => $page], $over),
            function ($v) { return $v !== '' && $v !== null; });
        return htmlspecialchars($self . ($p ? '?' . http_build_query($p) : ''), ENT_QUOTES);
    };
    ...
    foreach ($slice as $p) {
        ...
        $vo = $p['vip_override'] ?? null;
        $bo = $p['balance_override'] ?? null;
        $vipTag = ($vo !== null && $vo !== '') ? '<span class="badge owner">override</span>' : '<span class="badge">captured</span>';
        $balTag = ($bo !== null && $bo !== '') ? '<span class="badge owner">override</span>' : '<span class="badge">captured</span>';
        $rows .= '<tr>'
            . '<td><strong>' . $e($uname) . '</strong></td>'
            . '<td>' . $e($p['mobile_raw'] ?? '') . '</td>'
            . '<td>' . $e(player_effective_vip($p)) . ' ' . $vipTag . '</td>'
            . '<td>' . $e((string) player_effective_balance($p)) . ' ' . $balTag . '</td>'
            . '<td class="muted">' . $e($p['created_at'] ?? '') . '</td>'
            . '<td class="muted">' . $e($p['last_login_at'] ?? '') . '</td>'
            . '<td class="muted">' . $e($p['last_ip'] ?? '') . '</td>'
            . '<td style="white-space:nowrap">'
            . '<form method="post" action="' . $e($self) . '" style="display:inline">'
            . '<input type="hidden" name="csrf" value="' . $e($csrf) . '">'
            . '<input type="hidden" name="action" value="login_player">'
            . '<input type="hidden" name="username" value="' . $e($uname) . '">'
            . '<button class="btn sm" type="submit" title="Open a one-time auto-login session in this browser">Login</button>'
            . '</form> '
            . '<button class="btn ghost sm" type="button" data-copy="' . $e($uname) . '" onclick="pxCopy(this.dataset.copy)" title="Copy login ID">Copy ID</button>'
            . '</td></tr>';
        $rows .= '<tr><td colspan="8" style="padding-top:0">'
            . '<details><summary style="cursor:pointer;color:#9aa6c7">Edit overrides &amp; note</summary>'
            . '<form method="post" action="' . $e($self) . '" style="margin-top:8px">'
            . '<input type="hidden" name="csrf" value="' . $e($csrf) . '">'
            . '<input type="hidden" name="action" value="save_player">'
            . '<input type="hidden" name="username" value="' . $e($uname) . '">'
            . '<div class="row" style="gap:12px;align-items:flex-end;flex-wrap:wrap">'
            . '<div style="width:130px"><label>VIP override</label><input name="vip_override" value="' . $e($vo ?? '') . '" placeholder="VIP0"></div>'
            . '<div style="width:170px"><label>Balance override</label><input name="balance_override" value="' . $e($bo ?? '') . '" placeholder="0"></div>'
            . '<div style="flex:1;min-width:200px"><label>Note</label><input name="note" value="' . $e($p['note'] ?? '') . '" placeholder="internal note"></div>'
            . '<div><button class="btn ghost sm" type="submit">Save</button></div>'
            . '</div><p class="desc" style="margin:6px 0 0">Blank override clears it and falls back to the captured value.</p>'
            . '</form></details></td></tr>';
    }
    if ($rows === '') {
        $rows = '<tr><td colspan="8" class="muted">No players captured yet.</td></tr>';
    }

    $body = admin_notice_html($notice)
        . '<div class="card"><h3>Players</h3>'
        . '<div class="desc">Captured accounts. Passwords are stored only as password_hash(); the reversible vault copy is never shown.</div>'
        . '<form method="get" action="' . htmlspecialchars($self, ENT_QUOTES) . '" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">'
        . '<div style="flex:1;min-width:220px"><label>Search</label><input name="q" value="' . $e($q) . '" placeholder="username or mobile"></div>'
        . '<input type="hidden" name="sort" value="' . $e($sort) . '"><input type="hidden" name="dir" value="' . $e($dir) . '">'
        . '<div><button class="btn ghost" type="submit">Search</button></div>'
        . '<div><a class="btn ghost" href="' . $link(['export' => 'csv', 'page' => null]) . '">Export CSV</a></div>'
        . '</form>'
        . '<p class="desc" style="margin-top:10px">' . number_format($total) . ' player(s). '
        . 'Refresh is informational only &mdash; balance/VIP are captured from upstream responses while the player is active, '
        . 'and the proxy cannot query upstream without that player\'s session. Use <strong>Login</strong> for a live session.</p>'
        . '<table><thead><tr><th>Username</th><th>Mobile raw</th><th>VIP</th><th>Balance</th>'
        . '<th><a href="' . $link(['sort' => 'created', 'dir' => ($sort === 'created' && $dir === 'asc' ? 'desc' : 'asc'), 'page' => 1]) . '">Created</a></th>'
        . '<th><a href="' . $link(['sort' => 'last_login', 'dir' => ($sort === 'last_login' && $dir === 'asc' ? 'desc' : 'asc'), 'page' => 1]) . '">Last login</a></th>'
        . '<th>IP</th><th></th></tr></thead><tbody>' . $rows . '</tbody></table>'
        . '<div class="row" style="justify-content:space-between;margin-top:12px">'
        . '<a class="btn ghost" href="' . $link(['page' => max(1, $page - 1)]) . '">&larr; Prev</a>'
        . '<span class="muted">Page ' . $page . ' / ' . $pages . '</span>'
        . '<a class="btn ghost" href="' . $link(['page' => min($pages, $page + 1)]) . '">Next &rarr;</a>'
        . '</div></div>'
        . '<script>function pxCopy(t){try{navigator.clipboard&&navigator.clipboard.writeText(t);}catch(e){}}</script>';
    admin_layout($base, 'players', 'Players', $body, $_SESSION['px_user'] ?? '');
}
...
function handleAdmin(string $base, string $sub, array $cfg): void
{
    ...
    if ($method === 'POST' && admin_csrf_ok()) {
        switch ($postAction) {
            ...
            case 'save_player':
                $puser = trim((string) ($_POST['username'] ?? ''));
                $pknown = false;
                foreach (players_load() as $pp) {
                    if (strcasecmp((string) ($pp['username'] ?? ''), $puser) === 0) {
                        $pknown = true;
                        break;
                    }
                }
                $vipOv = trim((string) ($_POST['vip_override'] ?? ''));
                $balOv = trim((string) ($_POST['balance_override'] ?? ''));
                $note  = substr(trim((string) ($_POST['note'] ?? '')), 0, 300);
                if (!$pknown) {
                    $GLOBALS['admin_error'] = 'Unknown player.';
                } elseif ($vipOv !== '' && !preg_match('/^[A-Za-z0-9 _.-]{1,32}$/', $vipOv)) {
                    $GLOBALS['admin_error'] = 'VIP override contains invalid characters.';
                } elseif ($balOv !== '' && !preg_match('/^-?\d{1,15}(\.\d{1,4})?$/', $balOv)) {
                    $GLOBALS['admin_error'] = 'Balance override must be a number.';
                } else {
                    player_set_overrides($puser, [
                        'vip_override'     => $vipOv,
                        'balance_override' => $balOv,
                        'note'             => $note,
                    ]);
                    $notice = 'Player "' . $puser . '" updated.';
                    admin_audit_append(['action' => 'player_save', 'admin' => $_SESSION['px_user'] ?? '',
                        'target' => $puser, 'ip' => $_SERVER['REMOTE_ADDR'] ?? '']);
                }
                break;
            ...
            case 'login_player':
                ...
                $pIp = (string) ($_SERVER['REMOTE_ADDR'] ?? '');
                if (!$pknown) {
                    $GLOBALS['admin_error'] = 'Unknown player.';
                } elseif (!rate_limit_hit('player_login:' . $adminUser . ':' . $pIp, 10, 60)) {
                    admin_audit_append(['action' => 'player_login_rate_limited', 'admin' => $adminUser,
                        'target' => $puser, 'ip' => $pIp]);
                    $GLOBALS['admin_error'] = 'Too many login attempts. Try again in a minute.';
                } else {
                    $tokRow = player_token_mint($puser, $adminUser, $pIp, 60);
                    admin_audit_append(['action' => 'player_login', 'admin' => $adminUser, 'target' => $puser,
                        'ip' => $pIp, 'ua' => substr((string) ($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 200)]);
                    header('Location: ' . ($base === '' ? '' : $base) . '/player-login?token=' . urlencode($tokRow['token']), true, 302);
                    exit;
                }
                break;
    ...
    switch ($section) {
        ...
        case 'players':
            if ((string) ($_GET['export'] ?? '') === 'csv') {
                admin_export_players_csv();
            }
            admin_render_players($base, $notice);
            break;
        case 'settings':
